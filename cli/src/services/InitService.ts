...
export class InitService {
  ...
  getStackChoices(presets: Record<string, string[]>) {
    return Object.keys(presets)
      ...
      .map((k) => ({
        name: `${k.replace('stack:', '').toUpperCase()} — ${(presets[k] || []).join(', ')}`,
        value: k,
      }));
  }
  ...
  getAgentChoices(context: InitContext) {
    const groups = {
      popular: SUPPORTED_AGENTS.filter((a) => a.group === 'popular'),
      editor: SUPPORTED_AGENTS.filter((a) => a.group === 'editor'),
      platform: SUPPORTED_AGENTS.filter((a) => a.group === 'platform'),
    };

    const toChoice = (a: (typeof SUPPORTED_AGENTS)[0]) => ({
      name: a.name,
      value: a.id,
    ...
    return [
      ...
      ...groups.popular.map(toChoice),
      new inquirer.Separator('── Other Editors ────────────'),
      ...groups.editor.map(toChoice),
      new inquirer.Separator('── Platform Agents ──────────'),
      ...groups.platform.map(toChoice),
    ];
  }
  ...
  async buildAndSaveConfig(
    ...
    // Embed presets data into .skillsrc so expandPresets() can work without local file
    if (answers.roles && answers.roles.length > 0) {
      config.presets = answers.roles;
    }
    if (Object.keys(presets).length > 0) {
      config.presets_data = presets;
    }
